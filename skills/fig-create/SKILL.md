...
- **Container must never have a background fill:** The COMPONENT_SET wrapper and every COMPONENT variant frame must have `fills = []` — no background fill, ever. Components inherit their background from the canvas or parent frame. `/fig-qa` audits the container too; any fill on a COMPONENT or COMPONENT_SET node is a violation.
- **Container must use auto-layout with clip content disabled:** Every COMPONENT and COMPONENT_SET must have `layoutMode` set to `'HORIZONTAL'` or `'VERTICAL'` (never `'NONE'`) and `clipsContent = false`. Set both immediately after creating the node, before appending children.
- **Container spacing and radius must be token-bound:** The container's stroke, corner radius, padding, and gap must all be bound to variables — not hardcoded.
